...
const Login = () => {
  ...
  const handleSignup=async ()=>{
    ...
    try{
      const res=await axios.post("http://localhost:3005/signup" ,a)
  ...
  const handleSignin=async ()=>{
    try{
      //console.log(email)
      const a={"email":email?.current.value,"password":password?.current.value}
      const res=await axios.post("http://localhost:3005/signin" ,a)
      //console.log(res?.data)
      if (res){
        dispatch(adduser(res.data))
        navigate("/browser")
  ...
  return (
    <div className=''>
      ...
      <form className='absolute    bg-opacity-70  rounded-lg mt-12 left-1/3  text-white  bg-black p-12  w-3/12  ' onClick={(e)=>{e.preventDefault()}}>
      <h1 className="font-bold text-3xl py-4">
           Sign In
        </h1>
        <input ref={email} type="text"
          placeholder="Email Address" className='p-3 my-4 w-full bg-gray-700 rounded-lg' />
          <input ref={password} autoComplete='on' type="password"
          placeholder="password" className='p-3 my-4 w-full bg-gray-700 rounded-lg' />
        {issigninform ? <button className="p-3 my-6 bg-red-700 w-full rounded-lg" onClick={handleSignin}>Sign in</button> : <button className="p-3 my-6 bg-red-700 w-full rounded-lg" onClick={handleSignup}>Sign up</button>}
